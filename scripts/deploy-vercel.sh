...
# Deploy to Vercel (NO --prod flag = preview deployment)
printf "%-40s" "Deploying project"
DEPLOY_OUTPUT=$(mktemp)
if vercel deploy --cwd "$PROJECT_DIR" --scope "$VERCEL_TEAM_SLUG" $VERCEL_AUTH_ARGS --yes 2>&1 | tee "$DEPLOY_OUTPUT"; then
    printf "${GREEN}✅${NC}\n"
    echo ""
