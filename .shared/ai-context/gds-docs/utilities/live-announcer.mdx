...
import { Flex, Button, Tag, LiveAnnouncer, SearchField, Loader, Toast } from "@grammarly/design-system";
...
<ComponentStatusRow name="Live Announcer" />
...
<Hero
  isUtility
  path={useBaseUrl("/img/utilities/live-announcer-hero.png")}
  altText="Image search results for the word “sky” are displayed in a grid. An icon for accessibility is shown over a visual representation of screen reader text announced by the Live Announcer that says ‘188 images found matching “sky”.’"
/>
<p className="caption">
...
```tsx
import { LiveAnnouncer } from "@grammarly/design-system";
```
...
[
  {
    ...
    isRequired: true,
    description: 'The message content to announce.',
    defaultValue: null,
    typeDefinition: 'string'
  },
  {
    propertyName: 'assertiveness',
    isRequired: false,
    isDeprecated: false,
    description: `Controls the urgency of the announcement (whether or not to interrupt the user).`,
    defaultValue: "'polite'",
    typeDefinition: "'assertive' | 'polite'"
  },
  {
    propertyName: 'timeout',
    isRequired: false,
    isDeprecated: false,
    description: `How quickly to remove the message from the DOM.`,
    defaultValue: "7000",
...
[
  {
    propertyName: 'assertiveness',
    isRequired: true,
    description: 'The level of announcements to stop.',
    defaultValue: null,
    typeDefinition: "'assertive' | 'polite'"
  }
]} />
