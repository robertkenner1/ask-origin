...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      let thinkingConfig:
        | { type: "enabled" | "disabled"; budget_tokens: number }
        | undefined = undefined;
      if (thinking) {
        thinkingConfig = {
          type: "enabled",
        ...
        log(requestId, "Extended Thinking configuration:", {
          enabled: true,
      ...
      // Log if we have a custom system prompt
      if (systemPrompt) {
        log(requestId, "Using custom system prompt from config");
      }
