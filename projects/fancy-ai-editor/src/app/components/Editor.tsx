...
import { NewlyAdded } from "../extensions/TipTapNewlyAddedExtension";
...
export function Editor() {
  ...
  const [autoUnderlineEnabled, setAutoUnderlineEnabled] =
    useState<boolean>(true);
  const [underlineDensity, setUnderlineDensity] = useState<number>(20); // Percentage of words to underline
  ...
  const editor = useEditor({
    extensions: [
      ...
      AnimatedUnderline,
      // Add newly added content highlighting extension
      NewlyAdded,
    ],
  ...
  useEffect(() => {
    ...
    // Handle toggle underlines settings event from the Sidebar/SettingsDialog
    const handleToggleUnderlinesSettings = (event: Event) => {
      const customEvent = event as CustomEvent;
      console.log(
        "EDITOR: Received toggle underlines settings event:",
        customEvent,
      );

      if (customEvent.detail && typeof customEvent.detail.show === "boolean") {
        setShowUnderlinesSettings(customEvent.detail.show);
        console.log(
          "EDITOR: Updated underlines settings visibility:",
    ...
    document.addEventListener(
      "toggleUnderlinesSettings",
      handleToggleUnderlinesSettings,
    );
  ...
  // Add a function to simulate applying corrections to text
  const handleApplyCorrections = useCallback(() => {
    ...
    console.log(
      `Adding ${exampleCorrections.length} red underlines to specific words`,
    );

    // Apply corrections to the editor
    applyCorrections(editor, exampleCorrections);
  }, [editor, redUnderlineCount, findWordPositions]);
  ...
  // Add a function to apply animated underlines with dots
  const handleApplyAnimatedUnderlines = useCallback(() => {
    if (!editor) return;
    ...
    console.log(
      `Adding ${animatedRanges.length} animated underlines at sentence beginnings`,
    );

    // Apply animated underlines to the editor
    applyAnimatedUnderlines(editor, animatedRanges);
  }, [editor, animatedUnderlineCount]);
  ...
  // Function to automatically apply underlines based on density setting
  const applyAutoUnderlines = useCallback(() => {
    ...
    // Apply red underlines (errors) - avoid conflicts with animated underlines
    ...
    // Apply animated underlines (super enhancements) at sentence beginnings
    const animatedRanges: AnimatedUnderlineRange[] = [];

    // Find sentence beginnings for auto underlines
    const sentenceStartRegex = /(^|[.!?]\s+)(\w)/g;
    const sentenceStarts: number[] = [];
    ...
    // Apply animated underlines to sentence beginnings (limit to desired count)
    ...
    if (redCorrections.length > 0) {
      console.log("Applying red corrections:", redCorrections);
      applyCorrections(editor, redCorrections);
    }
    if (animatedRanges.length > 0) {
      console.log("Applying animated underlines:", animatedRanges);
      applyAnimatedUnderlines(editor, animatedRanges);
    }

    console.log(
      `Auto-applied underlines: ${redCorrections.length} red, ${animatedRanges.length} animated`,
    );
  }, [editor, autoUnderlineEnabled, underlineDensity, findWordPositions]);
  ...
  // Add state for controlling underlines settings visibility
  ...
  return (
    ...
    >
        ...
        <div className="bg-background sticky top-0 z-40 w-full">
          ...
          <div className="px-4 py-2 border-b flex justify-between items-center">
            <div className="flex gap-2">
              {/* Existing underlines settings content */}
            </div>

            {/* Assistant Toggle Button */}
            {/* <button
              onClick={() => setShowAssistant(!showAssistant)}
              className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-100 text-blue-800 hover:bg-blue-200 transition-colors"
            >
              <svg
                className="w-4 h-4"
                fill="none"
      ...
      {/* Main Editor Area with Assistant Panel */}
      <div className="relative flex flex-1 overflow-hidden">
        ...
        >
          ...
          <div
            ...
            style={{
              pointerEvents: "none",
              left: "50%",
              transform: `translateX(calc(-50% + var(--compose-bar-offset, 0px) - ${showAssistant ? "160px" : "0px"}))`,
            }}
          >
        ...
        {/* Assistant Panel */}
        {showAssistant && (
          <div className="w-80 bg-white border-l border-gray-200 flex-shrink-0 relative">
            ...
            <button
              ...
              className="absolute top-2 right-2 p-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition-colors z-10"
            >
              <svg
                className="w-4 h-4"
                fill="none"
      ...
      <style jsx global>{`
        ...
        .deep-writer-text {
          animation: rainbowToBlack 2s ease-out forwards;
        }
        ...
        .ProseMirror .newly-added-content {
          animation: rainbowToBlack 2s ease-out forwards;
        }
        ...
        // .ProseMirror .newly-added-content::before {
        //   content: '✨';
        //   position: absolute;
        //   top: -12px;
        //   left: 0;
        ...
        /* Animation for newly added content */
        ...
        /* Fade out transition when removing the class */
        // .ProseMirror .newly-added-content.removing {
        //   background: transparent;
        //   animation: fadeOutGlow 0.5s ease-out;
        // }

        // .ProseMirror .newly-added-content.removing::before {
        //   opacity: 0;
        //   transform: scale(0.8);
        ...
        .ProseMirror .correction-underline[data-type="style"]::after {
          background-color: #7d99f0; /* blue for style */
        }
        ...
        .ProseMirror .animated-underline::after {
          ...
          display: flex;
          align-items: center;
          // padding-right: 70px; /* Space for dots */
        }
